// Copyright (c) 2012 Luca Spiller
// CC-BY-SA 3.0

// FIXME: The source (in CoffeeScript) is available here: https://github.com/lucaspiller/88etag/blob/canvas2d/application.coffee, replace this with that and add a build system to compile everything to minified JS
...
LocalPlayer = (function(_super) {
  ...
  LocalPlayer.prototype.step = function() {
    // interface with DCPU
    if (Gt.onStep && this.ship) {
      Gt.onStep(this.ship);
    }
  };

  LocalPlayer.prototype.buildShip = function() {
    // run start callback to init DCPU
    if (Gt.onStart) {
      Gt.onStart();
    }
    return LocalPlayer.__super__.buildShip.apply(this, arguments);
  }

  LocalPlayer.prototype.respawn = function() {
    // player died, pause DCPU
    if (Gt.onStop) {
      Gt.onStop();
    }
